  ...
  <div class="guests-dropdown shadow-lg">
    <div v-for="(hab, index) in modelValue" :key="index" class="room-block">
      ...
      <div class="guest-row">
        ...
        <div class="counter">
          <button
            @click="updateGuest(index, 'adultos', -1)"
            :disabled="hab.adultos <= 1"
            class="btn-round"
          >
            —
          </button>
          <span class="count">{{ hab.adultos }}</span>
          <button @click="updateGuest(index, 'adultos', 1)" class="btn-round">
            +
          </button>
        </div>
      </div>
      ...
      <div class="guest-row">
        ...
        <div class="counter">
          <button
            @click="updateGuest(index, 'ninos', -1)"
            :disabled="hab.ninos <= 0"
            class="btn-round"
          >
            —
          </button>
          <span class="count">{{ hab.ninos }}</span>
          <button @click="updateGuest(index, 'ninos', 1)" class="btn-round">
            +
          </button>
        </div>
      </div>
